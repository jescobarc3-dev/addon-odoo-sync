    ...
    <Stack gap="lg">
      <Group justify="space-between" align="flex-start">
        <Box>
          ...
          <Text size="sm" c="#71717A">Sincroniza desde Odoo, activa usuarios y asigna permisos</Text>
        </Box>
        <Group gap="xs">
          <Button leftSection={<IconRefresh size={15} />} loading={syncing} onClick={handleSync} variant="default" size="sm">
            Sincronizar desde Odoo
          </Button>
          <Button leftSection={<IconPlus size={15} />} onClick={() => setModalCrear(true)} color="dark" size="sm">
            Crear usuario
          </Button>
        </Group>
      </Group>
              ...
              {usuarios.map((u) => (
                <tr key={u.id} style={{ borderBottom: '1px solid #F4F4F5' }}>
                  ...
                  <td style={{ padding: '10px 14px', textAlign: 'center' }}>
                    <Switch checked={u.activo} onChange={() => handleToggle(u)} size="sm" />
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <Group gap={4}>
                ...
                <tr>
                  <td colSpan={6} style={{ textAlign: 'center', padding: '48px 0' }}>
                    <Stack align="center" gap="xs">
                      ...
                      <Text c="#A1A1AA" size="sm">Sin usuarios. Usa "Sincronizar desde Odoo" para importarlos.</Text>
                    </Stack>
                  </td>
                </tr>
              )}
      ...
      <Modal opened={modalCrear} onClose={() => setModalCrear(false)} title="Crear usuario" size="md">
        <Stack gap="md">
          ...
          <Group justify="flex-end">
            ...
            <Button color="dark" size="sm" loading={creando} onClick={handleCrear}>Crear</Button>
          </Group>
        </Stack>
      </Modal>
      ...
      <Modal opened={!!editando} onClose={() => setEditando(null)} title={`Permisos — ${editando?.nombre}`} size="md">
        <Stack gap="md">
          ...
          <Text size="xs" c="#A1A1AA">
            ...
            <strong>cargar</strong> — subir documentos<br />
          ...
          <Group justify="flex-end">
            ...
            <Button color="dark" size="sm" onClick={handleGuardarPermisos}>Guardar</Button>
          </Group>
        </Stack>
      </Modal>
      ...
      <Modal opened={!!cambioPassword} onClose={() => setCambioPassword(null)} title={`Cambiar contraseña — ${cambioPassword?.nombre}`} size="sm">
        <Stack gap="md">
          ...
          <Group justify="flex-end">
            ...
            <Button color="dark" size="sm" onClick={handleCambiarPassword}>Guardar</Button>
          </Group>
        </Stack>
      </Modal>
    </Stack>
